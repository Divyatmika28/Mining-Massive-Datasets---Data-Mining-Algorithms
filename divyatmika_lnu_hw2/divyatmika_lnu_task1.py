...
if case==1:
	chunks = rdd.map(lambda x : (int(x[0]), int(x[1]))).groupByKey().mapValues(set) #user_id business_id

elif case==2:
	chunks = rdd.map(lambda x : (int(x[1]), int(x[0]))).groupByKey().mapValues(set) #business_id user_id 
...
if len(kvalues) != 0:
	...
	for i in range(0, len(kvalues)):
		...
		if isinstance(kvalues[i], str):
                    resFile.write(reduce1[i])
...
#phase1Items = sorted(reduce1, key = lambda item: (len(item), item))
